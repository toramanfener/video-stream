...
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("❌ Oynatılan yok")
        elif op == 1:
            await m.reply("✅ __Akış__ **Bitti.**\n\n**• Asistan sohbetten ayrıldı**")
        elif op == 2:
            await m.reply("🗑️ **Akış listesi temizlendi**\n\n**• Asistan sohbetten ayrıldı**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
                caption=f"⏭ **Geçerli parçaya atladı.**\n\n🏷 **İsim:** [{op[0]}]({op[1]})\n💭 **Chat:** `{chat_id}`\n💡 **Durum:** `Oynuyor`\n🎧 **Talep eden:** {m.from_user.mention()}",
...
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        ...
        await m.reply("❌ **Yayın bulunamadı**")
    ...
    if chat_id in QUEUE:
        try:
            ...
            await query.edit_message_text(
                "⏸ Yayın durduruldu", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Yayın bulunamadı", show_alert=True)
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.resume_stream(chat_id)
            await query.edit_message_text(
                "▶️ Yayın durduruldu", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
...
async def cbstop(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer("Anonim hesapsınız !\n\n» Lütfen anonim hesaptan çıkış yap.")
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("💡 Burayı kullanmanız için admin olmanız gerekiyor !", show_alert=True)
    chat_id = query.message.chat.id
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("✅ **Yayın bitti**", reply_markup=bcl)
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ Yayın bulunamadı", show_alert=True)
...
async def cbunmute(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer("Anonim hesapsın !\n\n» Lütfen anonim hesaptan çıkış yap.")
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("💡 Burayı kullanmanız için admin olmanız gerekiyor !", show_alert=True)
    chat_id = query.message.chat.id
    if chat_id in QUEUE:
        try:
            await call_py.unmute_stream(chat_id)
            await query.edit_message_text(
                "🔊 Asistanın başarıyla sesi açıldı", reply_markup=bttn
            )
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
